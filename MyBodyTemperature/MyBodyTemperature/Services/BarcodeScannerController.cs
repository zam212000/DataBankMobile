    ...
    internal class BarcodeScannerController : IBarcodeScannerService
    {
        ...
        private void ReadBarcodeInternal(Action<Result> onResult, Action<Exception> onError)
        {
            try
            {
                ...
                //Result = new Result(string.Empty, Array.Empty<byte>(), Array.Empty<ResultPoint>(), BarcodeFormat.All_1D);
               ...
               // Task.Delay(new TimeSpan(0, 0, 10)).Wait();
              //  OnScanResult(new Result(string.Empty, Array.Empty<byte>(), Array.Empty<ResultPoint>(), BarcodeFormat.All_1D));
